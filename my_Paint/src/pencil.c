...
static void verif_pixel(window_t *window, int i, int j, int pos[])
{
    if ((pos[0] - (window->cursor->thickness / 2) + i > IMG_POSITION_X &&
    pos[1] - (window->cursor->thickness / 2) + j > IMG_POSITION_Y) &&
    (pos[0] - (window->cursor->thickness / 2) + i <
    IMG_WIDTH_ + IMG_POSITION_X &&
    pos[1] - (window->cursor->thickness / 2) + j <
